...
import com.bigoffs.pdaremake.databinding.ActivityFindEpcByUnicodeBinding
import com.bigoffs.pdaremake.viewmodel.request.RequestFindGoodByEpcViewModel
import com.bigoffs.pdaremake.viewmodel.state.FindEpcByUnicodeViewModel
import com.blankj.utilcode.util.ToastUtils
import com.kingja.loadsir.core.LoadService
import me.hgj.jetpackmvvm.ext.parseState

class FindEpcByUniqueActivity() : BaseRfidFActivity<FindEpcByUnicodeViewModel, ActivityFindEpcByUnicodeBinding>(),
    Parcelable {
    ...
    override fun layoutId(): Int  = R.layout.activity_find_epc_by_unicode
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        loadsir = loadServiceInit(findViewById(R.id.ll_content)) {
            //点击重试时触发的操作
            loadsir.showLoading()
            requestViewModel.findEpcByUnicode(mViewModel.unique.value, mViewModel.shelfCode.value)
        }
        loadsir.showLoading()
        requestViewModel.findEpcByUnicode(mViewModel.unique.value, mViewModel.shelfCode.value)
    }
    ...
    override fun onFinish(data: String) {
        ...
        val epc: String = mViewModel.currentEpc.value
    ...
    override fun createObserver() {
        ...
        requestViewModel.uniqueData.observe(this, Observer {
            parseState(it, { data ->
                //成功

                mViewModel.data.value = data
                if (data.epc_code.isEmpty()){
                    ToastUtils.showShort("未找到匹配的EPC")
                    finish()
                }
                mViewModel.currentEpc.value = data.epc_code
...
//    }
    ...
    companion object CREATOR : Parcelable.Creator<FindEpcByUniqueActivity> {
        override fun createFromParcel(parcel: Parcel): FindEpcByUniqueActivity {
            return FindEpcByUniqueActivity(parcel)
        }

        override fun newArray(size: Int): Array<FindEpcByUniqueActivity?> {
