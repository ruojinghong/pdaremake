...
public class UfhData {
	...
	//    public static Map<String, Long> Result6c = new HashMap<String, Long>();   //暂时不使用Result6C --By Karl TAM
	...
	/*
        暂时不统计 -- By Karl Tam
        public static int cmdnum;   //标签数量
        public static long cmdsl;   //速率*/
...
/*            Date date1 = new Date();//创建现在的日期
			...
			for (int i = 0; i < scaned_num; i++) {
				key = lable[i].substring(0, lable[i].length() - 2);
				if (key == null || key.equals("")) return;

				rssi = lable[i].substring(lable[i].length() - 2);
				scanResult6c.put(key, Long.valueOf(rssi, 16));
/*
                //暂时不使用Result6c --By Karl TAM
	...
	public static void read6cOne(int selectedEd) {
		try {
			Date date = new Date();// 创建现在的日期
			long value = date.getTime();// 获得毫秒数
			String lable = UhfGetData.Scan6COne(selectedEd, target);

			epcs = lable.substring(0, lable.length() - 2);
		} catch (Exception e) {
			scanResult6c.clear();
		}
	}
	...
	public static void read6b() {
		...
		for (int i = 0; i < scaned_num; i++) {
			String key = lable[i];
			if (key == null || key.equals("")) return;
			Long num = scanResult6b.get(key) == null ? Long.valueOf(0) : scanResult6b.get(key);
...
//                        Log.i("yl", "length = " + Scan6CData[j]);
		...
		public static String Scan6COne(int Session, byte target) {
			try {
				int result = uhf.EPCC1G2_ScanEPC((byte) 0x06, (byte) Session,
						target);
				SoundFlag = false;
				if (result == 0) {
					SoundFlag = true;
